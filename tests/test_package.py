    ...
    def test_allowlist_checksum_and_reproducibility(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with ZipFile(archive) as bundle:
                ...
                self.assertIn('_KEngineMenu/Scripts/dmm_extension.lua', names)
                self.assertIn('_KEngineMenu/Scripts/navigation.lua', names)
                self.assertFalse(any('/dlls/' in n or n.lower().endswith('.dll') for n in names))
                self.assertIn('_KEngineMenu/Scripts/dirty_labels.lua', names)
                self.assertIn('_KEngineMenu/Scripts/init_config.lua', names)
                self.assertIn('_KEngineMenu/Scripts/fixes.lua', names)
                self.assertNotIn('_KEngineMenu/templates/fixes.lua', names)
                self.assertIn('_KEngineMenu/Scripts/settings_api.lua', names)
