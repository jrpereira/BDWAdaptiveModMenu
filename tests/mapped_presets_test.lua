...
assert(parsed[1].mmdMapping.values[2][1]==49)
...
assert(m.committed[2]==82 and m.mmdHiddenDirty[2] and m.mmdHiddenDirty[3])
m:set(2,50)
assert(m.pending[1]==0 and m.pending[2]==50 and m.pending[3]==1)
assert(not m.mmdHiddenDirty[2] and m.mmdHiddenDirty[3],'Manual edits must retain untouched preset suppression')
m:set(2,49)
assert(m.pending[1]==2 and m.mmdHiddenDirty[2],'Returning to the visual baseline restores its preset and hides its marker')
m:restore();assert(not m:dirty() and m.pending[2]==82 and next(m.mmdHiddenDirty)==nil)
m:set(1,2);m.failApply=true
local ok,err=m:apply();assert(not ok and err=='disk denied' and m:dirty() and m.mmdHiddenDirty[2])
m.failApply=false
local _,_,event=m:apply()
assert(not m:dirty() and event.values.Key==49 and event.changes.Mode.new==1)
assert(next(m.mmdHiddenDirty)==nil)
...
assert(m.pending[1]==2 and m.pending[2]==49 and m.mmdHiddenDirty[2],'Custom cannot be selected manually')
...
assert(not failed:dirty() and next(failed.mmdHiddenDirty)==nil)
